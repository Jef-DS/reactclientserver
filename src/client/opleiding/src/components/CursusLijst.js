...
class CursusForm extends Component {
    constructor(props) {
        super(props);
        this.state = { naam: "" };
...
class CursusLijstContainer extends Component {
    constructor(props) {
        super(props);
        this.state = {
            items: [],
    ...
    async handleFormSubmit(value) {
        const sendPost = await fetch('api/cursussen', {
            method: 'POST',
            body: JSON.stringify({
                naam: value
            }),
    ...
    render() {
        return (
            <React.Fragment>
                <Row>
                    <CursusForm handleSubmit={this.handleFormSubmit}></CursusForm>
                </Row>
                <Row>
                    <CursusLijst items={this.state.items} />
                </Row>
            </React.Fragment>
        );
    }
...
export default CursusLijstContainer;